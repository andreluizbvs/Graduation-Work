...
void ReadWrite::ReadParticles2D(vector<Particle2D>* particles,char* name)
{
	ifstream in;
	in.open(name);
...
void ReadWrite::WriteOut(vector<Particle2D>* particles,int number)
{
	FILE*out;
	char nome[1000] = "\0";
	int n = sprintf_s(nome,"saida_navegante//out%d.vtu",number);
